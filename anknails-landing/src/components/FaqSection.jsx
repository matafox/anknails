  ...
  const faqs = [
    {
      ...
      answer:
        i18n.language === "ru"
          ? `
После нажатия на кнопку **«Купить»** вы будете перенаправлены на страницу [Instagram](https://www.instagram.com/ank.a_studio),  
где сможете написать администратору для уточнения деталей и выбора способа оплаты.  

Оплата проходит безопасно и подтверждается вручную. После внесения оплаты вы получаете подтверждение и доступ к обучению.  

Также есть возможность забронировать место заранее - количество мест ограничено.
`
          : `
Після натискання кнопки **«Купити»** ви будете перенаправлені на сторінку [Instagram](https://www.instagram.com/ank.a_studio),  
де зможете написати адміністратору, щоб уточнити деталі й обрати зручний спосіб оплати.  

Оплата здійснюється безпечно та підтверджується вручну. Після внесення оплати ви отримуєте підтвердження та доступ до навчання.  

Також можна забронювати місце заздалегідь - кількість місць обмежена.
`,
    },
    {
      Icon: BookOpenCheck,
